...
__set_cdpath__() {
  ...
  cdpaths+=("${(@f)"$(< ~/.dotfiles/cdpaths)"}")
  [ -r ~/.dotfiles/local/cdpaths ] &&
    cdpaths+=("${(@f)"$(< ~/.dotfiles/local/cdpaths)"}")

  # Zsh does not perform tilde expansion on cdpath, so perform glob
  # substitution before adding them to cdpath.
  cdpath+=(${~cdpaths})
...
# Include any locally defined zshrc.
